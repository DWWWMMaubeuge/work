...
if( $_GET['idSkill'] != "" && $_GET['valSkill'] != "") {
    ...
    $q = $bdd->prepare('SELECT RESULTAT FROM Resultats WHERE ID_USER = :iduser AND ID_MATIERE = :idskill AND FORMATION = :formation AND ID_SESSION = :session AND MOIS = :mois');
    ...
    $q->bindParam(':session', $session, PDO::PARAM_INT);
