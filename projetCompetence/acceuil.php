...
if( $_POST && $_POST['email'] != "" && $_POST['password'] != "" ) 
{
    ...
    $req = "SELECT count(*) as nb FROM skills.users WHERE email='$email' AND password='$password'";
    $result = executeSQL( $req );
    $data = $result->fetch_assoc();
    if ( $data[ 'nb' ] == 1 )
    {
        $req = "SELECT * FROM skills.users WHERE email='$email' AND password='$password'";
        $result = executeSQL( $req );
        $data = $result->fetch_assoc();
...
<div class="login-box">
    ...
    <input class="btn" type="button" name="btn" value="Log in">
</div>
</FORM>
</body>
</html>