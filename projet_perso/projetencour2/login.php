...
if( $_POST && $_POST['mail'] != "" && $_POST['password']  ) 
{
    ...
    $req = "SELECT count(*) as nb FROM $dbname.utilisateur WHERE mail='$mail' AND password='$password' ";
    $result = executeSQL( $req );
    $data = $result->fetch(PDO::FETCH_ASSOC);
    if ( $data[ 'nb' ] == 1 )
    {
        $req = "SELECT * FROM $dbname.utilisateur WHERE mail='$mail' AND password='$password' ";
        $result = executeSQL( $req );
        $data = $result->fetch(PDO::FETCH_ASSOC);
